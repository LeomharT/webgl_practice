...
void main() {
  ...
  float intensity = opacity * 0.15;
  vec3 n = vec3(circles, sqrt(1. - dot(circles, circles)));

  vec2  rainUV    = n.xy * intensity;
  vec2  reflectUV = textureMatrix.xy / textureMatrix.w;
  vec2  finalUV   = reflectUV + normal.xy * uDistortionAmount - rainUV;
